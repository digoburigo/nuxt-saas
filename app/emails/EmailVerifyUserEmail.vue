...
import type { EmailVerifyUserEmailProps } from "./email-props";

const props = withDefaults(defineProps<EmailVerifyUserEmailProps>(), {
	email: "email@domain.com",
	token: "<TOKEN>",
});

const previewText = `Verificação do email ${props.email}`;
const verifyEmailLink = `http://localhost:3000/auth/verify-email?token=${props.token}`;
</script>
